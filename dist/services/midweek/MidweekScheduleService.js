...
class MidweekScheduleService {
    async getOrGenerateMonthSchedules(congregationId, year, month) {
        ...
        const workbookWeeks = await midweekWorkbookWeekRepository_1.midweekWorkbookWeekRepository.find({
            where: {
                weekDate: (0, typeorm_1.Between)(firstDayRange, lastDayRange)
            },
            relations: ["parts"]
        });
        const existingSchedules = await midweekScheduleRepository_1.midweekScheduleRepository.find({
            where: {
        ...
        existingSchedules.forEach(s => {
            if (s.parts) {
                s.parts.sort((a, b) => { var _a, _b; return ((_a = a.orderIndex) !== null && _a !== void 0 ? _a : 0) - ((_b = b.orderIndex) !== null && _b !== void 0 ? _b : 0); });
            }
            existingMap.set(s.weekDate, s);
        });
        const schedulesToReturn = [];
        for (const wbWeek of workbookWeeks) {
            const weekDayjs = (0, dayjs_1.default)(wbWeek.weekDate);
    ...
    async instantiateScheduleFromWorkbook(congregationId, wbWeek) {
        return await data_source_1.AppDataSource.transaction(async (manager) => {
            const schedule = new MidweekSchedule_1.MidweekSchedule();
            ...
            const savedSchedule = await manager.save(MidweekSchedule_1.MidweekSchedule, schedule);
            if (wbWeek.parts && wbWeek.parts.length > 0) {
                const meetingParts = wbWeek.parts.map(wp => {
                    const mp = new MidweekMeetingPart_1.MidweekMeetingPart();
                    ...
                    mp.room = MidweekMeetingPart_1.MidweekRoom.MAIN;
                    mp.orderIndex = wp.orderIndex;
                    mp.isActive = true;
                    mp.isCompleted = false;
                    return mp;
                });
                savedSchedule.parts = await manager.save(MidweekMeetingPart_1.MidweekMeetingPart, meetingParts);
            }
            else {
                savedSchedule.parts = [];
            }
    ...
    async getScheduleById(scheduleId, congregationId) {
    ...
    async updateSchedule(scheduleId, congregationId, data) {
        const schedule = await this.getScheduleById(scheduleId, congregationId);
        if (data.meetingDate !== undefined)
            schedule.meetingDate = data.meetingDate;
        if (data.isSpecial !== undefined)
            schedule.isSpecial = data.isSpecial;
        if (data.specialType !== undefined)
            schedule.specialType = data.specialType;
        if (data.specialName !== undefined)
            schedule.specialName = data.specialName;
        if (data.notes !== undefined)
            schedule.notes = data.notes;
        if (data.songOpen !== undefined)
            schedule.songOpen = data.songOpen;
        if (data.songMiddle !== undefined)
            schedule.songMiddle = data.songMiddle;
        if (data.songEnd !== undefined)
            schedule.songEnd = data.songEnd;
        ...
        if (clearsParticipants) {
            ...
            if (data.chairman_id !== undefined) {
                ...
                schedule.chairman = data.chairman_id ? { id: data.chairman_id } : null;
            }
            if (data.opening_prayer_id !== undefined) {
                schedule.opening_prayer_id = data.opening_prayer_id;
                schedule.openingPrayer = data.opening_prayer_id ? { id: data.opening_prayer_id } : null;
            }
            if (data.closing_prayer_id !== undefined) {
                schedule.closing_prayer_id = data.closing_prayer_id;
                schedule.closingPrayer = data.closing_prayer_id ? { id: data.closing_prayer_id } : null;
            }
            if (data.aux_counselor_1_id !== undefined) {
                schedule.aux_counselor_1_id = data.aux_counselor_1_id;
                schedule.auxCounselor1 = data.aux_counselor_1_id ? { id: data.aux_counselor_1_id } : null;
            }
            if (data.aux_counselor_2_id !== undefined) {
                schedule.aux_counselor_2_id = data.aux_counselor_2_id;
                schedule.auxCounselor2 = data.aux_counselor_2_id ? { id: data.aux_counselor_2_id } : null;
            }
            if (data.cbs_conductor_id !== undefined) {
                schedule.cbs_conductor_id = data.cbs_conductor_id;
                schedule.cbsConductor = data.cbs_conductor_id ? { id: data.cbs_conductor_id } : null;
            }
            if (data.cbs_reader_id !== undefined) {
                schedule.cbs_reader_id = data.cbs_reader_id;
                schedule.cbsReader = data.cbs_reader_id ? { id: data.cbs_reader_id } : null;
            }
        }
    ...
    async updatePart(partId, congregationId, data) {
        ...
        if (data.assigned_publisher_id !== undefined)
            part.assigned_publisher_id = data.assigned_publisher_id;
        if (data.assistant_publisher_id !== undefined)
            part.assistant_publisher_id = data.assistant_publisher_id;
        if (data.room !== undefined)
            part.room = data.room;
        if (data.title !== undefined)
            part.title = data.title;
        if (data.sourceMaterial !== undefined)
            part.sourceMaterial = data.sourceMaterial;
        if (data.timeMinutes !== undefined)
            part.timeMinutes = data.timeMinutes;
        if (data.lessonNumber !== undefined)
            part.lessonNumber = data.lessonNumber;
        if (data.studyPoint !== undefined)
            part.studyPoint = data.studyPoint;
        if (data.studyPointDescription !== undefined)
            part.studyPointDescription = data.studyPointDescription;
        if (data.brochure !== undefined)
            part.brochure = data.brochure;
        if (data.requiresAssistant !== undefined)
            part.requiresAssistant = data.requiresAssistant;
        if (data.method !== undefined)
            part.method = data.method;
        if (data.isActive !== undefined)
            part.isActive = data.isActive;
        if (data.isCompleted !== undefined)
            part.isCompleted = data.isCompleted;
        if (data.orderIndex !== undefined)
            part.orderIndex = data.orderIndex;
        await midweekMeetingPartRepository_1.midweekMeetingPartRepository.save(part);
        const updatedPart = await midweekMeetingPartRepository_1.midweekMeetingPartRepository.findOne({
            where: { id: partId },
            relations: ["assignedPublisher", "assistantPublisher"]
        });
        return updatedPart;
    }
    async createCustomPart(scheduleId, congregationId, partData) {
        const schedule = await this.getScheduleById(scheduleId, congregationId);
        const newPart = new MidweekMeetingPart_1.MidweekMeetingPart();
        newPart.schedule_id = schedule.id;
        newPart.section = partData.section || MidweekWorkbookPart_1.MidweekSection.LIVING;
        newPart.partType = partData.partType || MidweekWorkbookPart_1.MidweekPartType.CUSTOM;
        ...
        newPart.room = partData.room || MidweekMeetingPart_1.MidweekRoom.MAIN;
        ...
        return await midweekMeetingPartRepository_1.midweekMeetingPartRepository.save(newPart);
    }
    async deletePart(partId, congregationId) {
        const part = await midweekMeetingPartRepository_1.midweekMeetingPartRepository.findOne({
            where: { id: partId },
            relations: ["schedule"]
        });
        ...
        await midweekMeetingPartRepository_1.midweekMeetingPartRepository.remove(part);
    }
    async duplicateStudentPartsForRoom(scheduleId, congregationId, targetRoom) {
        ...
        for (const mainPart of mainStudentParts) {
            ...
            if (!exists) {
                const roomPart = new MidweekMeetingPart_1.MidweekMeetingPart();
        ...
        if (createdParts.length > 0) {
            return await midweekMeetingPartRepository_1.midweekMeetingPartRepository.save(createdParts);
        }
        return [];
    }
}
exports.MidweekScheduleService = MidweekScheduleService;
