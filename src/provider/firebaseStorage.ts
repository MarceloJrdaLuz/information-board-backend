...
export const storage = new Storage({
    ...
    keyFilename: './firebasestoragecredentials.json'
})

export const bucket = storage.bucket('information-board-36dd8.appspot.com')

export async function firebaseUpload(req: Request, res: Response, congregationNumber: string | null, saveBD: (file: NormalizeFiles) => void) {
    ...
    const key = `${congregationNumber ? congregationNumber : 'users-avatar'}/${v4()}-${fileName.replace(/\s/g, '')}`;
