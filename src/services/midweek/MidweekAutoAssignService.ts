...
import { MidweekPartType } from "../../entities/MidweekWorkbookPart";
...
import { MidweekScheduleService } from "./MidweekScheduleService";
import { MidweekSuggestionService, PublisherSuggestion } from "./MidweekSuggestionService";

export class MidweekAutoAssignService {
    ...
    private pickBest(
        ...
        assignedThisSession: Set<string>,
        allowConflict: boolean = false
    ): PublisherSuggestion | undefined {
        ...
        if (noSessionConflict) return noSessionConflict;
    ...
    async autoAssignSchedule(
        ...
        options: { chairmanPrays?: boolean } = { chairmanPrays: true }
    ): Promise<MidweekSchedule> {
        ...
        if (schedule.chairman_id) assignedThisSession.add(schedule.chairman_id);
        if (schedule.opening_prayer_id && schedule.opening_prayer_id !== schedule.chairman_id) assignedThisSession.add(schedule.opening_prayer_id);
        if (schedule.closing_prayer_id) assignedThisSession.add(schedule.closing_prayer_id);
        if (schedule.cbs_conductor_id) assignedThisSession.add(schedule.cbs_conductor_id);
        if (schedule.cbs_reader_id) assignedThisSession.add(schedule.cbs_reader_id);
        for (const p of schedule.parts) {
            if (p.assigned_publisher_id) assignedThisSession.add(p.assigned_publisher_id);
        }
        ...
        for (const part of schedule.parts) {
            if (part.partType === MidweekPartType.CBS || part.title.toLowerCase().includes("estudo bíblico")) {
                continue;
            }
            ...
            if (!part.assigned_publisher_id) {
                ...
                if (bestPartPub) {
                    ...
                    await midweekMeetingPartRepository.update(part.id, {
        ...
        await midweekScheduleRepository.update(schedule.id, {
